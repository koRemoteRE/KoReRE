...
void CPreRendering::createTextures() {
	...
	glGenTextures(1, &serverImgTexture);
...
void CPreRendering::setServerTexture(std::vector<unsigned char>& data) {
	cv::Mat im = cv::imdecode(data, 1);
	glBindTexture(GL_TEXTURE_2D,serverImgTexture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, im.cols, im.rows, 0, GL_BGR,
			GL_UNSIGNED_BYTE, im.data);
	serverImgWidth = im.cols;
	serverImgHeight = im.rows;
	glBindTexture(GL_TEXTURE_2D,0);
}
void CPreRendering::createFBO() {
	...
	glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT,
			GL_TEXTURE_2D, currentImgTexture, 0);
...
void CPreRendering::initGLSL() {
	...
	// Bind Attributes
	glBindAttribLocation(currentImgShaderProgram, SHADER_POSITION_LOC,
			"v_position");
	glBindAttribLocation(currentImgShaderProgram, SHADER_NORMAL_LOC,
			"v_normal");
	glBindAttribLocation(currentImgShaderProgram, SHADER_TEX_COORD_LOC,
			"v_texture");
	...
	// Bind Attributes
	glBindAttribLocation(warpingShaderProgram, SHADER_POSITION_LOC,
			"v_position");
